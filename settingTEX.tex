...
\extrainfo{注意：本公式书可能含有R18内容，且仅供娱乐}
...
常见的符咒类型：
\begin{itemize}
    \item \textbf{时域冲击符咒} $\delta$：使实用函数产生瞬时作用效果
          \[
...
\begin{definition}{二次元变换}
    给定三次元对象 \( h(x,y,z) \) 和OC特征向量 \(\bm{k} \in \mathbb{R}^n\)，其二次元变换 \( H(\mj u,\,\mj v) \) 定义为：
    \[
        H(\mj u,\,\mj v) = \iiint_{\mathbb{R}^3} h(x,y,z) \cdot \me^{-\mj \bm{k} \cdot \bm{p}} \,\md x\,\md y\,\md z
    \]
    其中：
    \begin{itemize}
        \item \(\bm{p}=\bm{p}(x,y,z)\) 是三—二次元转化函数
        \item \(\bm{k}\) 是OC的特征向量
...
\begin{example}
    ...
    \[ h(x,y,z) = \begin{cases}
        ...
        \end{cases} \]
    其二次元变换为：
    \[ H(\mj u,\,\mj v) = \int_0^1 \int_0^1 \int_0^1 \me^{-\mj (k_1x + k_2y + k_3z)} \md x\,\md y\,\md z \]
    计算结果为：
    \[ H(\mj u,\,\mj v) = \left(\frac{1 - \me^{-\mj k_1}}{\mj k_1}\right)\left(\frac{1 - \me^{-\mj k_2}}{\mj k_2}\right)\left(\frac{1 - \me^{-\mj k_3}}{\mj k_3}\right) \]
\end{example}
...
\begin{definition}{二次元反变换}
    ...
    其中：
    \begin{itemize}
        \item \(\bm{q}=\bm{q}(u,\,v)\) 是二—三次元转化函数
              % \item 核函数 \( e^{j \bm{k} \cdot \bm{q}} \) 补偿丢失的深度信息
    \end{itemize}
\end{definition}

\begin{theorem}[信息损失]
    数学上，二次元反变换是病态的(ill-posed)，因为：
    \[
        \dim(\mathbb{R}^2) < \dim(\mathbb{R}^3)
    \]
    需要正则化条件才能获得唯一解。
\end{theorem}

\begin{theorem}{OC反变换原理}{}
    在二次元中，若反变换对象为OC，则其反变换得到的三次元本家是唯一的。
    这是由于OC只属于唯一个体或群体。
\end{theorem}

\begin{example}
    给定二次元OC函数：
    \[ H(\mj u,\,\mj v) = \frac{\sin u}{u} \cdot \frac{\sin v}{v} \]
    其对应的三次元本家可以通过反变换得到：
    \[ h(x,y,z) = \iint_{\mathbb{R}^2} \frac{\sin u}{u} \cdot \frac{\sin v}{v} \cdot \me^{\mj (k_1u + k_2v)} \md u\,\md v \]
    在特定条件下，该反变换会还原出一个三维矩形函数。
\end{example}
